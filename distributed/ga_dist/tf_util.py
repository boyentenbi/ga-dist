...
def conv_block(x, dilation_rate, n_filters, kernel_size, activation):
    xf = CausalConv1D(x,
                      filters=n_filters,
...
def tc_block(x, len_req, n_filters):
    n_layers = np.ceil(np.log2(len_req))
    y = x
    for i in range(n_layers):
        y = conv_block(y, dilation_rate = 2**i, n_filters = n_filters)
